...
def draw_action_curve(surface, Xi, Yi, Thetai, UL, UR, color):
    ...
    while t < 1:
        ...
        if ((round(Xn), round(Yn)) in obstacles.OBSTACLE_POINTS) \
            or (round(Xn)/config.THRESHOLD_X, round(Yn)/config.THRESHOLD_Y, round(Thetan)/config.THRESHOLD_THETA) in visited_matrix_idx_set:
            return False
        points.append((int(Xn), int(Yn)))
    visited_matrix_idx_set.add((round(Xn)/config.THRESHOLD_X, round(Yn)/config.THRESHOLD_Y, round(Thetan)/config.THRESHOLD_THETA))
